module Make (F1 : Sig.F) (F2 : Sig.F with type letter = F1.letter) = struct
  ...
  let state_product states1 states2 =
    (* let _ = Printf.printf "States1: %d\n" (List.length states1) in *)
    (* let _ = Printf.printf "States2: %d\n" (List.length states2) in *)
  ...
  let equality_alphabets alphabet1 alphabet2 = 
    let alphabet1 = List.sort Letter.compare alphabet1 and alphabet2 = List.sort Letter.compare alphabet2 in
    List.equal (fun x y -> Letter.compare x y = 0) alphabet1 alphabet2

  (* Interface Implemantation *)

  let combine_and_exn dfa1 dfa2 =
    if Bool.not (equality_alphabets (F1.alphabet_list dfa1) (F2.alphabet_list dfa2)) then raise Alphabets_not_matching else 
    let new_states = state_product (F1.state_list dfa1) (F2.state_list dfa2) in
  ...
  let combine_or_exn dfa1 dfa2 =
    ...
    let new_accepting =
      (state_product
        (F1.state_list dfa1) (F2.accepting_list dfa2))
        @ (state_product (F1.accepting_list dfa1) (F2.state_list dfa2))
    in
    let new_transitions (s1, s2) letter =
      (F1.transitions dfa1 s1 letter, F2.transitions dfa2 s2 letter)
    in
